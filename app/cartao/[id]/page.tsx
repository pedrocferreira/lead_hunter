...
// ============================================================
// Cartão de visita online — link público compartilhável por 6 horas.
// Funciona em qualquer dispositivo móvel/desktop via WhatsApp/Direct.
// ============================================================
...
export default function CartaoPage() {
  ...
  useEffect(() => {
    ...
    // Busca dados no servidor (com validade de 6 horas)
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white gap-4">
        ...
        <p className="text-sm text-slate-400 font-medium animate-pulse">
          Carregando cartão de visita...
        </p>
      </div>
    );
  }

  // 2. Estado de Link Expirado (> 6 horas)
  if (isExpired) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white gap-6 px-6 text-center">
        ...
        <div className="max-w-md space-y-2">
          <h1 className="text-2xl font-bold text-white tracking-tight">
            Cartão de Visita Expirado
          </h1>
          <p className="text-sm text-slate-400 leading-relaxed">
            Este link temporário era válido por <strong>6 horas</strong> e já expirou.
            Para acessar ou gerar um novo cartão, utilize o painel de prospecção.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500 bg-slate-900 border border-slate-800 rounded-full px-4 py-2">
          ...
          <span>Links com validade de 6 horas para segurança e privacidade</span>
        </div>
  ...
  if (!lead) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-950 text-white gap-6 px-6 text-center">
        ...
        <div className="max-w-md space-y-2">
          <h1 className="text-2xl font-bold text-white">Cartão não encontrado</h1>
          <p className="text-sm text-slate-400">
            Este link pode ter expirado ou o endereço está incorreto.
          </p>
        </div>
  ...
  return (
    <main className="min-h-screen relative flex flex-col justify-between">
      {/* Top Banner sutil informando validade temporária */}
